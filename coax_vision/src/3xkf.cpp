...
,des_pos_x_origin(0.0),des_pos_y_origin(0.0),des_acc_z(0.0)
,R1(0.0001),R2(0.5),Q1(0.0001),Q2(0.0001),Q3(0.04)
...
bool CoaxVisionControl::setRawControl(double motor1, double motor2, double servo1, double servo2) {
  ...
  vicon_state.des_acc_z = state_z[2];//des_acc_z;
...
void CoaxVisionControl::set_hover(void){
    ...
    des_pos_x_origin = des_pos_x;
	  ...
	  gravity = gravity_sum/gravity_n;
		...
		std::cout<<initial_time<<std::endl;
		}
...
void CoaxVisionControl::stabilizationControl(void) {
  ...
  R << R1, 0,
	     0, R2;
  Eigen::Vector2f Z(sonar_z, accel[2]-gravity);

  state_previous = state_z;
  state_z = A*state_z;
  state_p = A*state_p*AT+Q;

  Eigen::Matrix2f S = H*state_p*HT+R;
	K = state_p*HT*S.inverse();
	Eigen::Vector2f Y = Z - H*state_z;
	state_z = state_z+K*Y;
...
void CoaxVisionControl::controlPublisher(size_t rate) {
	...
	while(ros::ok()) {
		...
		//		fout<<dt<<" "<<state_z(0)<<" "<<sonar_z<<" "<<global_z<<" "<<accel[2]<<" "<<gravity<<std::endl; 
			}
			if(stage==2){
				set_localize();
				stabilizationControl();
