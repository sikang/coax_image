...
,altitude_des(0.0)
,initial_des_pos(0),des_pos_x(0.0),des_pos_y(0.0)
{  
//	vicon_state_sub=node.subscribe("coax_57/vicon/odom",1,&CoaxVisionControl::viconCallback,this);
...
void CoaxVisionControl::loadParams(ros::NodeHandle &n) {
  ...
  n.getParam("altitudecontrol/k_pq",pm.k_pq);
...
bool CoaxVisionControl::rotorReady(void) {
	...
	if (rotor_ready_count < 150) {
		...
		motor2_des = 0;
		//		std::cout<<"motor1_des: "<<motor1_des<<std::endl;
		return false;
	}
		...
		//		std::cout<<"motor2_des: "<<motor2_des<<"  rotor_ready_count: "<<rotor_ready_count<<"pm.motor_const2: "<<pm.motor_const2<<std::endl;
		return false;
	}
	return true;	
}
...
void CoaxVisionControl::stabilizationControl(void) {
	double Dyaw,Dyaw_rate,Dyaw_int,yaw_control;
	...
	static double Daltitude_Int = 0,Dx_int = 0,Dy_int = 0;

	Eigen::Vector2f state = getState();

	altitude_des = rpyt_rc[3];
	//ROS_INFO("The altitude_des: [%f]",altitude_des);
	...
	Dx_int +=Dx;
	Dy_int +=Dy;
	Fdes_z = altitude_control+pm.m*9.84;
	Fdes_x = -pm.kp_xy*Dx-pm.kd_xy*Dx_rate-pm.ki_xy*Dx_int-pm.k_pq*twist_ang[1];
	Fdes_y = -pm.kp_xy*Dy-pm.kd_xy*Dy_rate-pm.ki_xy*Dy_int-pm.k_pq*twist_ang[0];
...
/*	Tlo=(pm.kM_up*Fdes_z+pm.kT_up*Mdes_z)/(pm.kT_up*pm.kM_lo+pm.kT_lo*pm.kM_up);
	if(Tup<0){
		Tup = 0;
}
	if(Tlo<0){
